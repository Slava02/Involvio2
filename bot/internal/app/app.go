...
func New(cfg *config.Config, uc usecase.UseCases) *Bot {
	return &Bot{
		cfg: cfg,
		uc:  uc,
	}
					...
					tm.NewHandler(tm.Or(tm.HasText(), tm.HasRegex("^"+constants.GroupBtn)),
						func(u *tm.Update) {
							...
							if u.Message.Text == constants.GroupBtn {
								//  TODO: handle error
								_, _ = b.uc.AddGroups(context.TODO(), user.UserName, models.DefaultGroup)
							} else {
								//  TODO: handle error
								newGroups, _ := b.uc.AddGroups(context.TODO(), user.UserName, u.EffectiveMessage().Text)
								if newGroups != "" {
									msg := tgbotapi.NewMessage(
										u.EffectiveChat().ID,
